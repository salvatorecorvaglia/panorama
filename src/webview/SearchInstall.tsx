...
import type { DepScope, Ecosystem, ProjectGroup, SearchResult } from '../core/types.js';
...
interface Props {
  ...
  onInstall: (name: string, version: string | null, scope: DepScope, manifestPath: string) => void;
  onUninstall: (name: string, ecosystem: Ecosystem, manifestPath: string) => void;
}
...
export function SearchInstall({
  ...
  const [targetManifest, setTargetManifest] = useState<string>(groups[0]?.manifestPath ?? '');
  ...
  const selectedGroup = groups.find((group) => group.manifestPath === targetManifest);
  ...
  return (
    <div className="search-panel">
      <div className="toolbar">
        <div className="toolbar__row">
          ...
          <select
            value={ecosystem}
            aria-label="Registry"
            onChange={(event) => setEcosystem(event.target.value as Ecosystem | 'all')}
          >
      ...
      {searching && results.length === 0 && <div className="empty">Searching…</div>}

      {!searching && query.trim().length >= 2 && results.length === 0 && !error && (
        <div className="empty">
          <h2>No packages found</h2>
          <p>
            Note that Go and PyPI have no official search API — for those, type the exact module
      ...
      {results.map((result) => {
        ...
        const compatible = !selectedGroup || selectedGroup.ecosystem === result.ecosystem;

        return (
          <div className="search-result" key={`${result.ecosystem}:${result.name}`}>
            <div className="search-result__info">
              <div className="search-result__name">
                <span>{result.name}</span>
                {result.version && <span className="muted">{result.version}</span>}
                <span className="badge">{ECOSYSTEM_LABELS[result.ecosystem]}</span>
                {result.deprecated && <span className="icon-warn">▲ deprecated</span>}
                {result.downloads !== undefined && (
                  <span className="muted">{formatDownloads(result.downloads)} downloads</span>
                )}
              </div>
              {result.description && (
              ...
              {installedElsewhere && installedElsewhere.length > 0 && (
                <div className="muted" style={{ fontSize: '0.85em' }}>
                  Already in {installedElsewhere.map((entry) => entry.projectLabel).join(', ')}
                </div>
              )}
            </div>
            ...
            <div className="search-result__actions">
              {installedHere ? (
                <>
                  ...
                  <button
                    className="danger"
                    onClick={() => onUninstall(result.name, result.ecosystem, targetManifest)}
                  >
                    Uninstall
                  </button>
                </>
                ...
                <button
                  ...
                  onClick={() =>
                    onInstall(result.name, result.version || null, scope, targetManifest)
                  }
                >
                  Install
                </button>
